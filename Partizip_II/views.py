...
from Verb.models import Perfekt
...
def play_game_partizip_II_multi(request, game_name='partizip_II', room_id=None):
    ...
    if not room.winner and room.status == 'playing':
        ...
        return render(request, 'partizip_II_game_multi.html', context=context)
    else:
        messages.warning(request, 'Game is over.', extra_tags='warning')
        return redirect('home')


def play_game_partizip_II_one(request, game_name='partizip_II'):
    if request.user.is_authenticated:
        ...
        if not request.session['winner']:
                    ...
                    if request.session['winner']:
                        for pl in players:
                            if pl['game_score'] >= MIN_SCORE:
                                ...
                                pz2_score = UserScores.objects.get(user=person).partizip_II_score
                                pz2_score += pl['game_score']
                                UserScores.objects.filter(user=person).update(partizip_II_score=pz2_score)
            ...
            return render(request, 'partizip_II_game_one.html', context=context)
        else:
            request.session['players'] = []
            return redirect(reverse('home'))
    else:
