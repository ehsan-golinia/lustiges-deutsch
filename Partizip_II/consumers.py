...
class Partizip_II_GameConsumer(WebsocketConsumer):
    ...
    def receive(self, text_data):
            ...
            if self.game_room.winner:
                for pl in self.game_room.player_states.all():
                    if pl.game_score >= self.MIN_SCORE or self.game_room.winner == pl.player:
                        pz2_score = UserScores.objects.get(user=pl.player).partizip_II_score
                        pz2_score += pl.game_score
                        UserScores.objects.filter(user=pl.player).update(partizip_II_score=pz2_score)
        ...
        event = {
            ...
            'quiz': {
                'infinitiv': rand_q.verb.infinitiv if rand_q else '',
                'english': rand_q.verb.english if rand_q else '',
                'turkish': rand_q.verb.turkish if rand_q else '',
                'partizip_II': rand_q.partizip_ii if rand_q else '',
            },
            'players': [
                {
                    'id': pl.player.id,
