...
def play_game_software_multi(request, game_name='software', room_id=None):
    ...
    if not room.winner and room.status == 'playing':
        ...
        return render(request, 'software_game_multi.html', context=context)
    else:
        messages.warning(request, 'Game is over.', extra_tags='warning')
        return redirect('home')


def play_game_software_one(request, game_name='software'):
    if request.user.is_authenticated:
        ...
        if not request.session['winner']:
            ...
            return render(request, 'software_game_one.html', context=context)
        else:
            request.session['players'] = []
            return redirect(reverse('home'))
    else:
