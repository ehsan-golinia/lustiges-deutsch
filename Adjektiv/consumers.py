...
from Adjektiv.models import Adjektiv
    ...
    def receive(self, text_data):
            ...
            if self.game_room.winner:
                for pl in self.game_room.player_states.all():
                    if pl.game_score >= self.MIN_SCORE or self.game_room.winner == pl.player:
                        adj_score = UserScores.objects.get(user=pl.player).adjektiv_score
                        adj_score += pl.game_score
                        UserScores.objects.filter(user=pl.player).update(adjektiv_score=adj_score)
        ...
        event = {
            ...
            'quiz': {
                ...
                'positiv': rand_q.positiv if rand_q else ''
            },
            'players': [
                {
                    'id': pl.player.id,
