...
from Adjektivdeklination.models import Adjektivdeklination, AdjKasus
...
class AdjektivdeklinationGameConsumer(WebsocketConsumer):
    def connect(self):
        self.this_user = self.scope['user']
        self.gameroom_id = self.scope['url_route']['kwargs']['gameroom_id']
        self.game_name = 'adjektivdeklination'
    ...
    def receive(self, text_data):
            ...
            if self.game_room.winner:
                for pl in self.game_room.player_states.all():
                    if pl.game_score >= self.MIN_SCORE or self.game_room.winner == pl.player:
                        adjDek_score = UserScores.objects.get(user=pl.player).adjektiv_deklination_score
                        adjDek_score += pl.game_score
                        UserScores.objects.filter(user=pl.player).update(adjektiv_deklination_score=adjDek_score)
        ...
        event = {
            ...
            'quiz': {
                'english': rand_q.adjDek.english if rand_q else '',
                'turkish': rand_q.adjDek.turkish if rand_q else '',
                'type': rand_q.adjDek.type if rand_q else '',
                'nom': rand_q.nom if rand_q else '',
                'akk': rand_q.akk if rand_q else '',
                'dat': rand_q.dat if rand_q else '',
                'gen': rand_q.gen if rand_q else '',
            },
            'players': [
                {
                    'id': pl.player.id,
