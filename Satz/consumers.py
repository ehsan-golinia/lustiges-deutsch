...
class SatzGameConsumer(WebsocketConsumer):
    def connect(self):
            ...
            if self.game_room.winner:
                for pl in self.game_room.player_states.all():
                    if pl.game_score >= self.MIN_SCORE or self.game_room.winner == pl.player:
                        sz_score = UserScores.objects.get(user=pl.player).satz_score
                        sz_score += pl.game_score
                        UserScores.objects.filter(user=pl.player).update(satz_score=sz_score)
