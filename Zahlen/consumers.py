...
from Zahlen.models import Zahlen
...
class ZahlenGameConsumer(WebsocketConsumer):
    ...
    def receive(self, text_data):
            ...
            if self.game_room.winner:
                for pl in self.game_room.player_states.all():
                    if pl.game_score >= self.MIN_SCORE or self.game_room.winner == pl.player:
                        zahl_score = UserScores.objects.get(user=pl.player).zahlen_score
                        zahl_score += pl.game_score
                        UserScores.objects.filter(user=pl.player).update(zahlen_score=zahl_score)
        ...
        event = {
            ...
            'quiz': {
                'number': rand_q.number if rand_q else '',
